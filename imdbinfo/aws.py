...
def build_everything(user_agent) -> dict:
    ...
    encoded_str = encoded.decode("utf-8", errors="strict") if isinstance(encoded, bytes) else encoded
    return {
        "checksum": checksum,
        "encoded": encoded_str,
...
# ====== PROOF OF WORK ======
def _check_bits(difficulty: int, hex_hash: str) -> bool:
    """Check that top `difficulty` bits of hex_hash are zero."""
...
def compute_pow(challenge_input: str, checksum: str, difficulty: int) -> str:
    """SHA-256 proof of work."""
    base = challenge_input + checksum
    for nonce in itertools.count(0):
        hash_hex = sha256_hashcash(base + str(nonce))
        if _check_bits(difficulty, hash_hex):
            return str(nonce)


def compute_scrypt(challenge_input: str, checksum: str, difficulty: int) -> str:
...
def get_filter_bytes(difficulty: int) -> int:
    sizes = {1: 1024, 2: 10 * 1024, 3: 100 * 1024, 4: 1048576, 5: 10 * 1048576}
    return sizes.get(difficulty, 0)


def compute_bandwidth(challenge_input: str, checksum: str, difficulty: int) -> str:
    """Bandwidth challenge - return base64 of null bytes."""
    n = get_filter_bytes(difficulty)
    return base64.b64encode(bytes(n)).decode("utf-8")


# Challenge type hash -> solver function
CHALLENGES = {
...
class AwsSolver:
    ...
    def _get_challenge_input(self, challenge_field) -> str:
        """
        The 'challenge' field from /inputs can be either:
          - a plain string  (older format)
          - a dict with an 'input' key  (newer format used by script 2)
        """
        if isinstance(challenge_field, dict):
            return challenge_field.get("input", "")
        return challenge_field  # already a string

    def _build_payload(self, inputs: dict, goku_props):
        challenge_type = inputs["challenge_type"]
        solver = CHALLENGES.get(challenge_type)
        if solver is None:
            raise ValueError(f"Unknown challenge type: {challenge_type}")

        payload = build_everything(user_agent=self.user_agent)
        challenge_input = self._get_challenge_input(inputs["challenge"])
        is_bandwidth = challenge_type == BANDWIDTH_CHALLENGE

        if is_bandwidth:
            # ── Bandwidth: multipart POST to /mp_verify ──────────────────────
            solution_b64 = solver(challenge_input, payload["checksum"], inputs["difficulty"])
            solution_metadata = {
            ...
            # ── PoW / scrypt: regular JSON POST to /verify ───────────────────
            solution = solver(challenge_input, payload["checksum"], inputs["difficulty"])
            return {
                "_is_bandwidth": False,
                "challenge": inputs["challenge"],
                "checksum": payload["checksum"],
    ...
    def post_payload(self, payload, host_url):
        if payload.get("_is_bandwidth"):
            ...
            clean = {k: v for k, v in payload.items() if k != "_is_bandwidth"}
            response = requests.post(
    ...
    def solve(self, site_html: str) -> str:
        ...
        return self.post_payload(payload, host_url)